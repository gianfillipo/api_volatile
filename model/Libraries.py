...
def captura(conn, cursor):
    ...
    if (tempo > 0):
        ...
        cursor.execute(
            "SELECT COUNT(idParametro) FROM `healthsystem`.`Parametro`")
        ...
        if Parametros < 1:
            ...
            for p in range(Equip):
                ...
                for i in range(Componentes):
                    ...
                    i = i + 1
                    cursor.execute("INSERT INTO `healthsystem`.`Parametro` (fkEquipamento, fkComponente, codigo, valid) VALUES (%s,%s,%s,%s);",
                                   (p, i, valor, validacao))
...
def validacaoMysql(conn):
    ...
    cursor.execute("CREATE DATABASE IF NOT EXISTS `healthsystem`;")
